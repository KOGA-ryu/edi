# Qt Widgets Draftsman Shell Architecture

`Main.cpp` composes major regions only: activity rail, left panel, main workspace, right panel, bottom panel, and status bar. Region internals live in `src/regions/`, reusable controls live in `src/components/`, feature surfaces live in `src/features/`, and shared state lives in `src/runtime/`.
...
Future integrations should start from `docs/design_philosophy.md`, `data/design_principles.json`, and `data/shell_surface_map.json`. The design files define the shared aesthetic and Dex role expectations; the surface map is the machine-readable index of shell surfaces, owner files, allowed feature slots, and proof coverage.

## Boundaries

- C++ is the human-editable UI surface.
- `UiStyle.cpp` owns colors, spacing, panel sizes, and typography.
...
- Runtime state belongs in `RuntimeController.cpp`.
...
- Reuse integration belongs in the consumer app build/data configuration, not copied C++ files.
