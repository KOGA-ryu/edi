...
| ui-20260618-spatial-phase1 | edi-ui | builder (integration) | open — SOLE integration owner, dungeon spatial-model inversion Phase 1. Serialize ALL hot-file changes (DraftingMapTypes/DraftingRoom/DraftingSerialize/MapToonExport/createMapFromSpec) through edi-ui to local master. Merge dungeon-map slices IN ORDER: regression-lock golden (CANARY) → syncGraphForMovedObject → additive structs; edi-gate each. CANARY baseline TOON sha256 6c632293 (rooms12/plugs26/conn12); non-byte-identical drift = HALT+flag. Awaiting first green slice. | [docs/handoffs/ui-20260618-spatial-phase1.md](ui-20260618-spatial-phase1.md) | edi-ui-planner |
