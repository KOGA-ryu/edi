...
| ui-20260619-cleanup-assetlink | edi-ui | builder | open — HUB cleanup wave. D06 ✅ (06b9e2c dead RecentFilesStore) + D04 ✅ (d990313 headless offscreen fallback + smoke) + blender-lab MERGE ✅ (6799f74, 120/120) already landed on master. Reviewer gate SETTLED all three remaining (ae476647481d6f9c2): D17 controller→edi_controller_core (7 sibling .cpp are EMPTY shells; lib = Controller+Projection+Store+full set; 8 targets link it; behavior-preserving compile-once WIN), D08 live AssetZoo on EdiShellWindow (best-effort load, empty-on-miss, getter, NO UI), D07 on-demand validateAssetRefs(zoo) const query (NOT on modelChanged; edi_asset_link PRIVATE on lib). Sequence D17→D08→D07. NEXT: brief builder batch. DEFERRED look decisions for user: zoo panel + ref-warning surface. | [docs/handoffs/ui-20260619-cleanup-assetlink.md](ui-20260619-cleanup-assetlink.md) | hub |
