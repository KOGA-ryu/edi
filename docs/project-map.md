...
|------|-----------|:-----:|-----------------------------|
| **A — Blender → asset** | recipe TOML → bind/resolve → deterministic bpy → headless render | **engine solid · lab UI real & interactive (◐)** | The recipe engine parses/binds/resolves (refuses to compile an unresolved stream — proof never guesses)/compiles to bpy, and `ProcessRunStore` spawns headless Blender → PNG. ✅ The Blender workspace IS the lab: canvas (Main) + object list (Left) + a **Right tabbing Palette \| Render \| Compiled** + a **bottom terminal tabbing Steps \| Editor \| ASCII Proof**, all live off the `opsStreamChanged` seam. ✅ **Click-compose, both halves:** the **step Palette** (`56bcf9c`) chains steps by clicking (Box/Cylinder/Sphere/Ring), the **Steps op inspector** (`80bd665`) tunes a selected step's numeric fields — the HUMAN composes by clicking, the TOML re-syncs for the AI ("toml is for ai to edit"). ✅ **Full op CRUD + editors:** delete/reorder steps (`ad7126f`); every scalar field editable — enum/material/string/int/bool, not just numbers (`dd9f93c`); a **binding picker** — right-click a number → bind to a drafted measurement (`-the binding commit-`); **pop-out** any proof pane into a floating window (`-pop-out-`). ✅ **R2 mesh proof** (`-r2-`): `edi_craft.py --obj-out` writes a deterministic OBJ (28 objects, cutters included) with no Blender — committed golden. ◐ **Custom craftsmen** — Python foundation done: `edi_craft.py` scans a `craftsmen/` folder, reads each script's `MANIFEST`, renders a `Script` op via the script's `proof_mesh`, and `--list-craftsmen` emits the registry for the lab to read (sample: `twisted_column`). Remaining = the C++ slice: a generic `ScriptOp` in the variant + store + the palette/inspector wiring. Plus moulding-sequence editing; frameless pop-out skin. |
...
| **Blender recipe lab / script composer** | **Blender workspace** (Right Palette\|Render\|Compiled + bottom Steps\|Editor\|ASCII); add/tune/delete/reorder steps, all field types, binding picker, pop-out panes; OBJ mesh proof via CLI | **missing** | ◐ (composer complete; custom craftsmen next) |
