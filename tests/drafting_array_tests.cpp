...
int main()
{
    ...
    EDI_CHECK(repeatX);
    EDI_CHECK(repeatX->copyCount == 5);
    EDI_CHECK(nearlyEqual(repeatX->spacingX, 0.25));
    EDI_CHECK(nearlyEqual(repeatX->spacingY, 0.0));

    const std::optional<DraftingArrayRepeatSettings> repeatY = draftingArrayRepeatSettingsFromAxisId("y", 2, 0.25, 0.05);
    EDI_CHECK(repeatY);
    EDI_CHECK(repeatY->copyCount == 2);
    EDI_CHECK(nearlyEqual(repeatY->spacingX, 0.0));
    EDI_CHECK(nearlyEqual(repeatY->spacingY, 0.05));
    EDI_CHECK(!draftingArrayRepeatSettingsFromAxisId("diagonal", 3, 0.1, 0.1));
    ...
    EDI_CHECK(repeatedLine.ok);
    EDI_CHECK(repeatedLine.objects.size() == 3);
    EDI_CHECK(repeatedLine.objects[0].id == "repeat_1");
    EDI_CHECK(repeatedLine.objects[0].kind == DraftingShapeKind::Line);
    EDI_CHECK(repeatedLine.objects[0].metadata.toolProvenance == "repeat");
    EDI_CHECK(repeatedLine.objects[0].metadata.source == "line_1");
    EDI_CHECK(repeatedLine.objects[0].stroke.color == "#55ccaa");
    ...
    EDI_CHECK(firstLine != nullptr);
    EDI_CHECK(thirdLine != nullptr);
    EDI_CHECK(nearlyEqual(firstLine->a.x, 0.2));
    EDI_CHECK(nearlyEqual(firstLine->a.y, 0.2));
    EDI_CHECK(nearlyEqual(thirdLine->a.x, 0.4));
    EDI_CHECK(nearlyEqual(thirdLine->b.x, 0.7));
    ...
    EDI_CHECK(repeatedCircle.ok);
    const auto *secondCircle = std::get_if<CircleGeometry>(&repeatedCircle.objects[1].geometry);
    EDI_CHECK(secondCircle != nullptr);
    EDI_CHECK(nearlyEqual(secondCircle->center.x, 0.25));
    EDI_CHECK(nearlyEqual(secondCircle->center.y, 0.65));
    EDI_CHECK(nearlyEqual(secondCircle->radius, 0.1));
    ...
    EDI_CHECK(!invalidGuideRepeat.ok);
    EDI_CHECK(invalidGuideRepeat.code == DraftingResultCode::InvalidGeometry);

    auto empty = repeatDraftingObject(line, {}, 0.1, 0.0);
    EDI_CHECK(!empty.ok);
    EDI_CHECK(empty.code == DraftingResultCode::InvalidGeometry);

    auto zeroSpacing = repeatDraftingObject(line, {"zero_repeat"}, 0.0, 0.0);
    EDI_CHECK(!zeroSpacing.ok);
    EDI_CHECK(zeroSpacing.code == DraftingResultCode::InvalidGeometry);

    auto badSpacing = repeatDraftingObject(line, {"bad_repeat"}, std::numeric_limits<double>::infinity(), 0.0);
    EDI_CHECK(!badSpacing.ok);
    EDI_CHECK(badSpacing.code == DraftingResultCode::InvalidGeometry);

    auto duplicateIds = repeatDraftingObject(line, {"duplicate_repeat", "duplicate_repeat"}, 0.1, 0.0);
    EDI_CHECK(!duplicateIds.ok);
    EDI_CHECK(duplicateIds.code == DraftingResultCode::DuplicateObjectId);

    auto sourceIdReuse = repeatDraftingObject(line, {"line_1"}, 0.1, 0.0);
    EDI_CHECK(!sourceIdReuse.ok);
    EDI_CHECK(sourceIdReuse.code == DraftingResultCode::DuplicateObjectId);
    ...
    // Grid array: source occupies cell (0,0); copies fill the rest row-major.
    {
        ...
        EDI_CHECK(grid.ok);
        EDI_CHECK(grid.objects.size() == 5);
        EDI_CHECK(grid.objects[0].metadata.toolProvenance == "grid_array");
        EDI_CHECK(grid.objects[0].metadata.source == "dot_1");
        EDI_CHECK(grid.objects[0].stroke.color == "#112233");
        // Row 0: cells (1,0) and (2,0).
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[0].geometry).point.x, 0.3));
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[0].geometry).point.y, 0.3));
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[1].geometry).point.x, 0.4));
        // Row 1: cells (0,1), (1,1), (2,1).
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[2].geometry).point.x, 0.2));
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[2].geometry).point.y, 0.5));
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[4].geometry).point.x, 0.4));
        EDI_CHECK(nearlyEqual(std::get<PointGeometry>(grid.objects[4].geometry).point.y, 0.5));

        // A single column needs no X spacing — it is a vertical run.
        auto column = gridArrayDraftingObject(dot, {"c_1", "c_2"}, 1, 3, 0.0, 0.1);
        EDI_CHECK(column.ok);
        EDI_CHECK(column.objects.size() == 2);
        ...
        EDI_CHECK(!gridArrayDraftingObject(dot, {"bad_1"}, 3, 2, 0.1, 0.2).ok);
        EDI_CHECK(!gridArrayDraftingObject(dot, {}, 1, 1, 0.1, 0.1).ok);
        EDI_CHECK(!gridArrayDraftingObject(dot, {"bad_2"}, 0, 2, 0.1, 0.1).ok);
        EDI_CHECK(!gridArrayDraftingObject(dot, {"bad_3", "bad_4"}, 3, 1, 0.0, 0.1).ok);
        EDI_CHECK(!gridArrayDraftingObject(dot, {"bad_5"}, 2, 1,
                                        std::numeric_limits<double>::infinity(), 0.0).ok);
        EDI_CHECK(!gridArrayDraftingObject(dot, {"dot_1"}, 2, 1, 0.1, 0.0).ok);
        ...
        EDI_CHECK(!guideGrid.ok);
        EDI_CHECK(guideGrid.code == DraftingResultCode::InvalidGeometry);
        auto guideRadial = radialArrayDraftingObject(guideSource, {"gr_1"}, {0.5, 0.7});
        EDI_CHECK(!guideRadial.ok);
        EDI_CHECK(guideRadial.code == DraftingResultCode::InvalidGeometry);
    }
    ...
    // ring (copies + source) around the centre; geometry is only translated.
    {
        ...
        EDI_CHECK(radial.ok);
        EDI_CHECK(radial.objects.size() == 3);
        EDI_CHECK(radial.objects[0].metadata.toolProvenance == "radial_array");
        EDI_CHECK(radial.objects[0].metadata.source == "ring_1");
        ...
        EDI_CHECK(quarter != nullptr);
        EDI_CHECK(nearlyEqual(quarter->center.x, 0.5));
        EDI_CHECK(nearlyEqual(quarter->center.y, 0.3));
        EDI_CHECK(nearlyEqual(quarter->radius, 0.05)); // size never changes
        const auto *half = std::get_if<CircleGeometry>(&radial.objects[1].geometry);
        EDI_CHECK(nearlyEqual(half->center.x, 0.7));
        EDI_CHECK(nearlyEqual(half->center.y, 0.5));
        const auto *threeQuarter = std::get_if<CircleGeometry>(&radial.objects[2].geometry);
        EDI_CHECK(nearlyEqual(threeQuarter->center.x, 0.5));
        EDI_CHECK(nearlyEqual(threeQuarter->center.y, 0.7));
        ...
        EDI_CHECK(lineRadial.ok);
        const auto *flipped = std::get_if<LineGeometry>(&lineRadial.objects[0].geometry);
        EDI_CHECK(flipped != nullptr);
        EDI_CHECK(nearlyEqual(flipped->a.x, 0.6));
        EDI_CHECK(nearlyEqual(flipped->a.y, 0.45));
        EDI_CHECK(nearlyEqual(flipped->b.x, 0.8));
        EDI_CHECK(nearlyEqual(flipped->b.y, 0.35));

        // Rejections: no copies; non-finite centre; centre on the object
        // (every copy would land exactly on the source).
        EDI_CHECK(!radialArrayDraftingObject(ringCircle, {}, {0.5, 0.5}).ok);
        EDI_CHECK(!radialArrayDraftingObject(ringCircle, {"r_bad"},
                                          {std::numeric_limits<double>::quiet_NaN(), 0.5}).ok);
        auto degenerate = radialArrayDraftingObject(ringCircle, {"r_bad"}, {0.3, 0.5});
        EDI_CHECK(!degenerate.ok);
        EDI_CHECK(degenerate.code == DraftingResultCode::InvalidGeometry);
    }
    ...
    // 90/180/270: rotationDeg advances per spoke AND the centre orbits arm 0.25.
    {
        ...
        EDI_CHECK(rosette.objects.size() == 3);
        EDI_CHECK(rosette.objects[0].metadata.toolProvenance == "rotate_copies");
        EDI_CHECK(rosette.objects[0].metadata.source == "rc_src");
        ...
        for (std::size_t i = 0; i < 3; ++i) {
            ...
            EDI_CHECK(r != nullptr);
            EDI_CHECK(nearlyEqual(r->rotationDeg, expectedAngle[i])); // turns with the spoke
            EDI_CHECK(nearlyEqual(r->width, 0.1) && nearlyEqual(r->height, 0.1)); // scale 1.0
            ...
            EDI_CHECK(nearlyEqual(std::hypot(cx - 0.5, cy - 0.5), arm));
        }
        ...
        EDI_CHECK(nearlyEqual(first->origin.x + first->width / 2.0, 0.5));
        EDI_CHECK(nearlyEqual(first->origin.y + first->height / 2.0, 0.75));
        ...
        EDI_CHECK(!dup.ok);
        EDI_CHECK(dup.code == DraftingResultCode::DuplicateObjectId);
        EDI_CHECK(!rotateCopiesDraftingObject(square, {}, {0.5, 0.5}, 360.0).ok);
        EDI_CHECK(!rotateCopiesDraftingObject(square, {"x"},
        ...
        EDI_CHECK(!guideRosette.ok);
        EDI_CHECK(guideRosette.code == DraftingResultCode::InvalidGeometry);
        ...
        EDI_CHECK(!zeroAngle.ok);
        EDI_CHECK(zeroAngle.code == DraftingResultCode::InvalidGeometry);
    ...
    // arc-length-spaced stations of the path, optionally rotated to the tangent.
    {
        ...
        EDI_CHECK(lineNoTan.ok);
        EDI_CHECK(lineNoTan.objects.size() == 3);
        EDI_CHECK(lineNoTan.objects[0].metadata.toolProvenance == "array_along_curve");
        EDI_CHECK(lineNoTan.objects[0].metadata.source == "aac_src");
        ...
        EDI_CHECK(c0 != nullptr && c1 != nullptr && c2 != nullptr);
        // stations: x = 0.0, 0.5, 1.0; all y = 0
        EDI_CHECK(nearlyEqual(c0->center.x, 0.0) && nearlyEqual(c0->center.y, 0.0));
        EDI_CHECK(nearlyEqual(c1->center.x, 0.5) && nearlyEqual(c1->center.y, 0.0));
        EDI_CHECK(nearlyEqual(c2->center.x, 1.0) && nearlyEqual(c2->center.y, 0.0));
        EDI_CHECK(nearlyEqual(c0->radius, 0.05)); // size is preserved
        ...
        EDI_CHECK(lineTan.ok);
        EDI_CHECK(lineTan.objects.size() == 2);
        for (const DraftingObject &obj : lineTan.objects) {
            ...
            EDI_CHECK(nearlyEqual(r->rotationDeg, 45.0));
        }
        ...
        // First station is the arc's start (1,0), last is the arc's end (0,1).
        {
            ...
            EDI_CHECK(nearlyEqual(pt0->point.x, 1.0) && nearlyEqual(pt0->point.y, 0.0));
            EDI_CHECK(nearlyEqual(pt2->point.x, 0.0) && nearlyEqual(pt2->point.y, 1.0));
        }

        // -- Rejections ---------------------------------------------------
        // Empty id list.
        auto emptyIds = arrayAlongCurve(srcCircle, {}, hLine, false);
        EDI_CHECK(!emptyIds.ok);
        EDI_CHECK(emptyIds.code == DraftingResultCode::InvalidGeometry);
        ...
        EDI_CHECK(!circleReject.ok);
        EDI_CHECK(circleReject.code == DraftingResultCode::InvalidGeometry);
        ...
        EDI_CHECK(!rectReject.ok);
        EDI_CHECK(rectReject.code == DraftingResultCode::InvalidGeometry);

        // Zero-length path.
        const DraftingGeometry zeroLine{LineGeometry{{0.5, 0.5}, {0.5, 0.5}}};
        auto zeroLen = arrayAlongCurve(srcCircle, {"rej_2"}, zeroLine, false);
        EDI_CHECK(!zeroLen.ok);
        EDI_CHECK(zeroLen.code == DraftingResultCode::InvalidGeometry);
    }
