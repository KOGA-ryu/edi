...
int main()
{
    // S0: a fresh document carries an empty block library.
    {
        ...
        EDI_CHECK(document.blocks.empty());
    }
    ...
    // origin-based union extent, preserving the members' relative layout.
    {
        ...
        EDI_CHECK(block.id == "block_0001");
        EDI_CHECK(block.name == "table");
        EDI_CHECK(block.assetRef.empty()); // S1: no asset ref unless one is passed
        EDI_CHECK(block.objects.size() == 2);
        ...
        EDI_CHECK(linked.assetRef == "recipe.oak_chair");

        // Cached bounds: lower-left at the origin, extent = the source union span.
        EDI_CHECK(nearly(block.bounds.x, 0.0) && nearly(block.bounds.y, 0.0));
        EDI_CHECK(nearly(block.bounds.width, 0.3) && nearly(block.bounds.height, 0.15));
        ...
        EDI_CHECK(nearly(line.a.x, 0.0) && nearly(line.a.y, 0.0));
        EDI_CHECK(nearly(line.b.x, 0.3) && nearly(line.b.y, 0.0));
        const auto &circle = std::get<CircleGeometry>(block.objects[1].geometry);
        EDI_CHECK(nearly(circle.center.x, 0.2) && nearly(circle.center.y, 0.1));
        EDI_CHECK(nearly(circle.radius, 0.05));
        ...
        EDI_CHECK(nearly(srcLine.a.x, 0.3) && nearly(srcLine.a.y, 0.4));
    }
    ...
    // degenerate cases (empty id, duplicate id, empty objects).
    {
        ...
        EDI_CHECK(addBlock(document, block).ok);
        EDI_CHECK(document.blocks.size() == 1);
        ...
        EDI_CHECK(!emptyId.ok && emptyId.code == DraftingResultCode::EmptyObjectId);

        // Duplicate id.
        auto dup = addBlock(document, buildBlockFromObjects("block_0001", "again", sampleSources()));
        EDI_CHECK(!dup.ok && dup.code == DraftingResultCode::DuplicateObjectId);

        // Empty objects: a definition must define something.
        DraftingBlock empty = buildBlockFromObjects("block_0002", "nothing", {});
        EDI_CHECK(empty.objects.empty());
        auto emptyObjects = addBlock(document, empty);
        EDI_CHECK(!emptyObjects.ok && emptyObjects.code == DraftingResultCode::InvalidSelectionTarget);
    ...
    // and a second remove of the same id is rejected.
    {
        ...
        EDI_CHECK(removeBlock(document, "block_0001").ok);
        EDI_CHECK(document.blocks.empty());
        auto gone = removeBlock(document, "block_0001");
        EDI_CHECK(!gone.ok && gone.code == DraftingResultCode::ObjectNotFound);
    }
    ...
    // an empty library — additive-tolerant, so no document-version bump.
    {
        ...
        EDI_CHECK(restored.value->blocks.size() == 1);
        const DraftingBlock &rb = restored.value->blocks.front();
        EDI_CHECK(rb.id == "block_0001" && rb.name == "table");
        EDI_CHECK(rb.assetRef == "recipe.tavern_table"); // Seam-B link survives the round-trip
        EDI_CHECK(rb.objects.size() == 2);
        // Bounds are recomputed on load (never stored), so they survive the trip.
        EDI_CHECK(nearly(rb.bounds.x, 0.0) && nearly(rb.bounds.y, 0.0));
        EDI_CHECK(nearly(rb.bounds.width, 0.3) && nearly(rb.bounds.height, 0.15));
        ...
        // key and must decode to an empty ref (no version bump needed).
        {
            ...
            EDI_CHECK(tolerant.value->blocks.front().assetRef.empty());
        }
        ...
        EDI_CHECK(stripped.value->blocks.empty());
    }
    ...
    // (the pre-existing plug/connection collision this closed too).
    {
        ...
        EDI_CHECK(highestDocumentIdSerial(document) == 7);

        DraftingPlug plug;
        plug.id = "plug_0009";
        document.plugs.push_back(plug);
        EDI_CHECK(highestDocumentIdSerial(document) == 9);

        document.objects.push_back(makeDraftingObject("line_0003", DraftingShapeKind::Line,
                                                      LineGeometry{{0.0, 0.0}, {1.0, 1.0}}));
        EDI_CHECK(highestDocumentIdSerial(document) == 9); // plug still highest
    }
    ...
    // id-bearing vector must participate in highestDocumentIdSerial.
    {
        ...
        EDI_CHECK(highestDocumentIdSerial(document) == 11);

        // Adding a lower-serial plug doesn't displace the node's serial.
        DraftingPlug plug;
        plug.id = "plug_0005";
        document.plugs.push_back(plug);
        EDI_CHECK(highestDocumentIdSerial(document) == 11); // node still highest

        // A block with a higher serial takes the lead.
        DraftingBlock block;
        block.id = "block_0015";
        document.blocks.push_back(block);
        EDI_CHECK(highestDocumentIdSerial(document) == 15);
    }
