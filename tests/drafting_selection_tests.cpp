...
int main()
{
    ...
    EDI_CHECK(addLayer(document, makeDraftingLayer("locked_visible", "Locked Visible", 3)).ok);

    EDI_CHECK(addObject(document, object("inside_line", DraftingShapeKind::Line, LineGeometry{{0.1, 0.1}, {0.2, 0.2}})).ok);
    EDI_CHECK(addObject(document, object("crossing_line", DraftingShapeKind::Line, LineGeometry{{0.45, 0.45}, {0.7, 0.7}})).ok);
    EDI_CHECK(addObject(document, object("outside_line", DraftingShapeKind::Line, LineGeometry{{0.8, 0.8}, {0.9, 0.9}})).ok);
    ...
    EDI_CHECK(addObject(document, hiddenObject).ok);
    ...
    EDI_CHECK(addObject(document, hiddenLayerObject).ok);
    ...
    EDI_CHECK(addObject(document, lockedVisibleLayerObject).ok);
    EDI_CHECK(updateLayerFlags(document, "locked_visible", true, true).ok);

    const std::vector<DraftingObjectId> selected = selectableObjectsInBounds(document, Bounds2D{0.0, 0.0, 0.5, 0.5});
    EDI_CHECK(contains(selected, "inside_line"));
    EDI_CHECK(contains(selected, "crossing_line"));
    EDI_CHECK(contains(selected, "locked_visible_layer_object"));
    EDI_CHECK(!contains(selected, "outside_line"));
    EDI_CHECK(!contains(selected, "hidden_object"));
    EDI_CHECK(!contains(selected, "hidden_layer_object"));

    const std::vector<DraftingObjectId> emptySelection = selectableObjectsInBounds(document, Bounds2D{0.0, 0.0, 0.0, 0.0});
    EDI_CHECK(emptySelection.empty());
    ...
    EDI_CHECK(invalidSelection.empty());
