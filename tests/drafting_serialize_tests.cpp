...
void assertDocumentsEqual(const DraftingDocument &a, const DraftingDocument &b)
{
    ...
    for (std::size_t i = 0; i < a.connections.size(); ++i) {
        ...
        EDI_CHECK(ca.locked == cb.locked && ca.keyId == cb.keyId);
    }
}
...
int main()
{
    ...
    // wall_visual above, which is why this needed no document-version bump.
    {
        ...
        EDI_CHECK(decoded.value->connections.size() == 2);
        // D15: the lock survives the byte round-trip; the unlocked edge stays clear.
        EDI_CHECK(!decoded.value->connections[0].locked && decoded.value->connections[0].keyId.empty());
        EDI_CHECK(decoded.value->connections[1].locked && decoded.value->connections[1].keyId == "gold_key");
        ...
        for (const auto &entry : gv.mapValue) {
            ...
            for (const auto &dm : entry.second.mapValue) {
                ...
                EDI_CHECK(!unlockedHasLock && !unlockedHasKey); // key absent entirely
                EDI_CHECK(lockedHasLock && lockedHasKey);       // locked edge carries both
            }
        }
