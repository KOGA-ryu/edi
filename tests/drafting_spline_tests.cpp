...
int main()
{
    ...
    EDI_CHECK(validateGeometry(spline).ok);
    {
        EDI_CHECK(!validateGeometry(SplineGeometry{{{0.1, 0.1}}}).ok); // one point
        ...
        EDI_CHECK(validateGeometry(SplineGeometry{{{0.1, 0.1}, {0.4, 0.4}}}).ok);
    }

    // kind round-trips through the variant and the name maps.
    EDI_CHECK(geometryKind(spline) == DraftingShapeKind::Spline);
    EDI_CHECK(std::string(shapeKindName(DraftingShapeKind::Spline)) == "spline");
    EDI_CHECK(shapeKindFromName("spline") == DraftingShapeKind::Spline);
    ...
    // starts at the first knot and ends at the last. 4 points = 3 segments.
    {
        ...
        EDI_CHECK(sampled.size() == 1 + 3 * 16);
        EDI_CHECK(nearlyEqual(sampled.front().x, 0.1) && nearlyEqual(sampled.front().y, 0.5));
        EDI_CHECK(nearlyEqual(sampled.back().x, 0.9) && nearlyEqual(sampled.back().y, 0.4));
        for (const Point2D &knot : spline.controlPoints) {
            EDI_CHECK(sampleContains(sampled, knot));
        }
        // Fewer than three points: nothing to curve — returned verbatim.
        const SplineGeometry two{{{0.1, 0.1}, {0.4, 0.4}}};
        EDI_CHECK(sampleSpline(two).size() == 2);
        EDI_CHECK(sampleSpline(SplineGeometry{}).empty());
    }
    ...
    // (the curve runs through all of them).
    {
        ...
        for (const Point2D &knot : spline.controlPoints) {
            EDI_CHECK(knot.x >= b.x - 0.000001 && knot.x <= b.x + b.width + 0.000001);
            EDI_CHECK(knot.y >= b.y - 0.000001 && knot.y <= b.y + b.height + 0.000001);
        }
    }
    ...
    // recomputed from them.
    {
        ...
        EDI_CHECK(moved.controlPoints.size() == spline.controlPoints.size());
        for (std::size_t i = 0; i < moved.controlPoints.size(); ++i) {
            EDI_CHECK(nearlyEqual(moved.controlPoints[i].x, spline.controlPoints[i].x + 0.05));
            EDI_CHECK(nearlyEqual(moved.controlPoints[i].y, spline.controlPoints[i].y - 0.1));
        }
    }
    ...
    // point is large.
    {
        ...
        EDI_CHECK(hitDistance(object.geometry, {0.6, 0.7}) < 0.0001); // a knot, on the curve
        EDI_CHECK(hitDistance(object.geometry, {2.0, 2.0}) > 1.0);
    }
    ...
    // handles: one read-only knot marker per control point, at the control point.
    {
        ...
        EDI_CHECK(anchors.size() == spline.controlPoints.size());
        EDI_CHECK(anchors[0].id == "control_0");
        EDI_CHECK(nearlyEqual(anchors[0].point.x, 0.1) && nearlyEqual(anchors[0].point.y, 0.5));

        const auto descriptors = draftingHandlesForObject(object);
        EDI_CHECK(descriptors.size() == spline.controlPoints.size());
        EDI_CHECK(descriptors[2].id == "control_2" && descriptors[2].readOnly);
    }
    ...
    // control points — the SAME gesture as polyline, a different geometry kind.
    {
        ...
        EDI_CHECK(draftingToolKindFromId("spline_tool") == DraftingToolKind::Spline);
        EDI_CHECK(std::string(draftingToolKindName(DraftingToolKind::Spline)) == "spline");
        const DraftingObjectBuildResult built = buildDraftingObjectForTool(request);
        EDI_CHECK(built.ok && built.object.kind == DraftingShapeKind::Spline);
        const auto &geo = std::get<SplineGeometry>(built.object.geometry);
        EDI_CHECK(geo.controlPoints.size() == 3);
        EDI_CHECK(nearlyEqual(geo.controlPoints[2].x, 0.8) && nearlyEqual(geo.controlPoints[2].y, 0.1));
        ...
        EDI_CHECK(!buildDraftingObjectForTool(single).ok);
    }
    ...
    // serialize round-trip: the control points survive (MsgPack array).
    {
        ...
        EDI_CHECK(g.controlPoints.size() == 3);
        EDI_CHECK(nearlyEqual(g.controlPoints[1].x, 0.45) && nearlyEqual(g.controlPoints[1].y, 0.65));
    }
