...
int main()
{
    ...
    assert(addDuplicateOrderLayer.ok);
    assert(layerDocument.layers.size() == 3);
    assert(layerDocument.layers[0].id == "default");
    assert(layerDocument.layers[0].order == 0);
    assert(layerDocument.layers[1].id == "sketch");
    assert(layerDocument.layers[1].order == 1);
    assert(layerDocument.layers[2].id == "notes");
    assert(layerDocument.layers[2].order == 2);
    const auto revisionBeforeMoveLayer = layerDocument.revision;
    auto moveDefaultLayerUp = moveLayer(layerDocument, "default", 1);
    assert(moveDefaultLayerUp.ok);
    assert(layerDocument.layers[0].id == "sketch");
    assert(layerDocument.layers[0].order == 0);
    assert(layerDocument.layers[1].id == "default");
    assert(layerDocument.layers[1].order == 1);
    assert(layerDocument.layers[2].id == "notes");
    assert(layerDocument.layers[2].order == 2);
    assert(layerDocument.revision == revisionBeforeMoveLayer + 1);
    const auto revisionBeforeNoopLayerMove = layerDocument.revision;
    auto moveNotesLayerUpAtTop = moveLayer(layerDocument, "notes", 1);
    assert(moveNotesLayerUpAtTop.ok);
    assert(layerDocument.revision == revisionBeforeNoopLayerMove);
    auto moveMissingLayer = moveLayer(layerDocument, "missing_layer", 1);
    assert(!moveMissingLayer.ok);
    assert(moveMissingLayer.code == DraftingResultCode::LayerNotFound);
    ...
    assert(updateSketchPlot.ok);
    sketchLayer = findLayer(layerDocument, "sketch");
    assert(sketchLayer != nullptr);
    assert(!sketchLayer->plot.plotEnabled);
    assert(sketchLayer->plot.penId == "pen_blue");
    assert(sketchLayer->plot.strokeColor == "#75c7ff");
    assert(sketchLayer->plot.strokeWidth == 1.0);
    assert(layerDocument.revision == revisionBeforePlotStyle + 1);
    ...
    assert(sameSketchPlot.ok);
    assert(layerDocument.revision == revisionBeforeSamePlotStyle);
    ...
    assert(!invalidPlot.ok);
    assert(invalidPlot.code == DraftingResultCode::InvalidGeometry);
    auto missingLayerPlot = updateLayerPlotStyle(layerDocument, "missing_layer", sketchPlot);
    assert(!missingLayerPlot.ok);
    assert(missingLayerPlot.code == DraftingResultCode::LayerNotFound);
