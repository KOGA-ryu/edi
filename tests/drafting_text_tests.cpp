...
int main()
{
    ...
    EDI_CHECK(validateGeometry(text).ok);
    {
    ...
    EDI_CHECK(geometryKind(text) == DraftingShapeKind::TextAnnotation);
    EDI_CHECK(std::string(shapeKindName(DraftingShapeKind::TextAnnotation)) == "text");
    EDI_CHECK(shapeKindFromName("text") == DraftingShapeKind::TextAnnotation);

    // bounds: a box at the position; width grows with content length.
    {
        const Bounds2D b = computeBounds(DraftingGeometry{text});
        EDI_CHECK(nearlyEqual(b.x, 0.3) && nearlyEqual(b.y, 0.4));
        EDI_CHECK(nearlyEqual(b.height, 0.1));
        EDI_CHECK(nearlyEqual(b.width, 5 * 0.1 * 0.55)); // "Hello" = 5 chars
        // empty content collapses to the minimum width, not zero.
        const TextAnnotationGeometry empty{{0.0, 0.0}, "", 0.1};
        EDI_CHECK(nearlyEqual(computeBounds(DraftingGeometry{empty}).width, 0.1 * 0.3));
    }
    ...
    // translate moves the position; content and height are unchanged.
    {
        ...
        EDI_CHECK(nearlyEqual(moved.position.x, 0.4) && nearlyEqual(moved.position.y, 0.2));
        EDI_CHECK(moved.content == "Hello" && nearlyEqual(moved.height, 0.1));
    }

    // hit: a point inside the box is near; a far point is positive.
    {
        const DraftingObject object = textObject(text);
        EDI_CHECK(hitDistance(object.geometry, {0.4, 0.45}) < 0.0001); // inside the box
        EDI_CHECK(hitDistance(object.geometry, {2.0, 2.0}) > 1.0);     // far away
    }
    ...
    // preserves the content (a move that dropped the string would fail here).
    {
        ...
        EDI_CHECK(handles.size() == 1 && handles[0].id == "text_position");
        EDI_CHECK(nearlyEqual(handles[0].point.x, 0.3) && nearlyEqual(handles[0].point.y, 0.4));
        ...
        EDI_CHECK(nearlyEqual(moved.position.x, 0.6) && nearlyEqual(moved.position.y, 0.7));
        EDI_CHECK(moved.content == "Hello");
    }
    ...
    // numeric edit: setting the height by field id.
    {
        ...
        EDI_CHECK(e.ok && nearlyEqual(std::get<TextAnnotationGeometry>(e.geometry).height, 0.2));
    }
    ...
    // a default-content text annotation at the click point.
    {
        ...
        EDI_CHECK(built.ok && built.object.kind == DraftingShapeKind::TextAnnotation);
        const auto geo = std::get<TextAnnotationGeometry>(built.object.geometry);
        EDI_CHECK(nearlyEqual(geo.position.x, 0.25) && nearlyEqual(geo.position.y, 0.25));
        EDI_CHECK(geo.content == "Text" && nearlyEqual(geo.height, 0.04));
    }
    ...
    // serialize round-trip: the STRING content survives (MsgPack text, incl. UTF-8).
    {
        ...
        EDI_CHECK(g.content == "Round trip \xE2\x9C\x93");
        EDI_CHECK(nearlyEqual(g.height, 0.06));
    }
