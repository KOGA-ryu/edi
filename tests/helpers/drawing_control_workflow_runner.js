...
function checkMax(record, budget, budgetField, recordField, label, recordIndex, bucket) {
    if (budget[budgetField] !== undefined) {
        ...
        if (actual > limit) {
            bucket.push(budgetFailure(label, recordIndex, recordField, actual, limit))
        }
    }
}
...
function checkMaxPerPointerMove(record, budget, budgetField, recordField, label, recordIndex, bucket) {
    if (budget[budgetField] !== undefined && Number(record.pointerMoves) > 0) {
        const actual = Number(record[recordField]) / Number(record.pointerMoves)
        const limit = Number(budget[budgetField])
        if (actual > limit) {
            bucket.push(budgetFailure(label, recordIndex, `${recordField}PerPointerMove`, actual, limit))
        }
    }
}
...
function scriptMetadata(workflow) {
    return {
...
function runFixture(repoRoot, workflow, options) {
    const fixtureName = String(workflow && workflow.fixture || "")
    const executable = options && options.executable ? options.executable : path.join(repoRoot, "build", "qt_qml_region_split")
    ...
    const result = spawnSync(executable, [
        ...
        cwd: repoRoot,
        encoding: "utf8",
        timeout: Number(options && options.timeoutMs || 10000),
    })
    ...
    if (result.error) {
        ...
        return {
            ...
            fixture: fixtureName,
            metadata: scriptMetadata(workflow),
            ok: false,
...
function runWorkflows(repoRoot, workflows, options) {
    return workflows.map(workflow => runFixture(repoRoot, workflow, options || {}))
}
