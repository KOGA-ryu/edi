...
int main(int argc, char **argv)
{
    ...
    // -------------------------------------------------------------------------
    {
        ...
        assert(seamCLocked.find("connections[12]{from,to,type,locked,key_id}:\n") != std::string::npos);
        // The locked edge is the first authored connection: entrance.up -> room1.entrance.
        assert(seamCLocked.find("  entrance.up,room1.entrance,corridor,true,gold_key\n") != std::string::npos);
        // An unlocked edge emits the columns with the neutral defaults.
        assert(seamCLocked.find("  room1.to_2,room2.to_1,corridor,false,\"\"\n") != std::string::npos);
        ...
        assert(seamBLocked.find("connections[12]{from,to,type,locked,key_id}:\n") != std::string::npos);
        assert(seamBLocked.find("  entrance.up,room1.entrance,corridor,true,gold_key\n") != std::string::npos);
    }
